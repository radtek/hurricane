  ...
  public class FuseDhtHelper {
    ...
    public void GetProc(object ostate) {
      ...
      for (; item_count == 0 && retries > 0; retries--) {
        ...
        results = _dht.Get(dht_key_bytes);
        item_count = results.ResultEntries.Count;
        if (item_count == 0) {
          Logger.WriteLineIf(LogLevel.Verbose, _log_props, string.Format(
              "Got nothing"));
        } else {
      ...
      var result = results.ResultEntries[0];
      DictionaryData dd = null;
      try {
        dd = DictionaryData.CreateDictionaryData(result.Value);
      } catch (Exception ex) {
      ...
      if (dd is FragmentationInfo) {
        ...
        results = new DhtResults();
        DhtResultEntry entry = ConvertToDhtResultEntry(bde);
      ...
      DateTime dt = DateTime.MinValue;
      foreach (var entry in results.ResultEntries) {
        RegularData rd = (RegularData)DictionaryData.CreateDictionaryData(entry.Value);
        DhtDataFile file = new DhtDataFile(s_parent_path, rd.PayLoad, (int)entry.MetaInfo["age"], (int)entry.MetaInfo["ttl"]);
        file.WriteToFile();
        DateTime end = DateTime.UtcNow - new TimeSpan(0, 0, file.Age) + new TimeSpan(0, 0, file.TTL);
        if (dt == DateTime.MinValue || end < dt) {
          //if hasn't been set, set it. Otherwise only set if we can get a smaller datetime
    ...
    private static DhtResultEntry ConvertToDhtResultEntry(BrunetDhtEntry bde) {
      DhtResultEntry entry = new DhtResultEntry(bde.Value);
      entry.MetaInfo["age"] = bde.Age;
      entry.MetaInfo["ttl"] = bde.Ttl;
      return entry;
    }
