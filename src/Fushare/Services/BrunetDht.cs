...
namespace Fushare.Services {
  /**
   * Derives the the DhtBase with the Implementation against Brunet Dht.
   * 
   */
  public partial class BrunetDht : DhtBase {
    private IXmlRpcDht _dht;
    private static IDictionary _log_props = Logger.PrepareLoggerProperties(typeof(BrunetDht));
    public const int DefaultBrunetTtl = 3600;  //Default ttl : 1 hour
    /// <summary>
    ...
    /// <summary>
    /// Puts name/value pair into Brunet DHT
    /// </summary>
    /// <param name="ttl">Time-To-Live in seconds</param>
    public override void Put(byte[] key, byte[] value, int ttl) {
      Put(key, value, ttl, BrunetMaxPieceSize, true);
    }

    /// <summary>
    /// Same as Put except that it fails in case of name collision
    /// </summary>
    /// <param name="ttl">Time-To-Live in seconds</param>
    ...
    public override DhtResults Get(byte[] key) {
      ...
      DhtResults results = ConvertToDhtResults(dgrs);
      return results;
    }
    ...
    private static DhtResults ConvertToDhtResults(DhtGetResult[] dgrs) {
      DhtResults results = new DhtResults();
      foreach (DhtGetResult dgr in dgrs) {
        var resultEntry = new DhtResultEntry(dgr.value);
